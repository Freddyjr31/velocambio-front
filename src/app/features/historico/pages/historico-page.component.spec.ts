...
describe('HistoricoPageComponent', () => {
  ...
  function createFixture(): ComponentFixture<HistoricoPageComponent> {
    getHistoricoBcv.and.callFake((page: number) => of(PAGES[page]));
  ...
  it('en la primera carga pide la página 1 directamente', () => {
    const fixture = createFixture();

    expect(getHistoricoBcv).toHaveBeenCalledOnceWith(1, PAGE_SIZE);
  ...
  it('Siguiente navega hacia fechas más antiguas', () => {
    const fixture = createFixture();
    ...
    nextButton(fixture).click();
    fixture.detectChanges();
    expect(getHistoricoBcv.calls.mostRecent().args).toEqual([3, PAGE_SIZE]);
